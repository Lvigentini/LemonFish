<template>
  <div class="env-setup-panel">
    <div class="scroll-container">
      ...
      <!-- Step 03: generatedual-platform simulationconfig -->
    ...
    <!-- Phase 3: Pre-flight token estimate modal -->
    <PreflightModal
      :visible="preflightVisible"
      :simulation-id="simulationId"
      :agents="preflightAgents"
      :rounds="preflightRounds"
      :document-chars="preflightDocChars"
      :report-sections="4"
      @close="preflightVisible = false"
      @confirm="proceedStartSimulation"
    />
  </div>
</template>
...
// State
const phase = ref(0) // 0: initialise, 1: generatepersona, 2: generateconfig, 3: complete
...
const preflightAgents = computed(() => profiles.value.length || 0)
const preflightRounds = computed(() => {
  if (useCustomRounds.value) return customMaxRounds.value
  return autoGeneratedRounds.value || 10
})
const preflightDocChars = computed(() => {
  return props.projectData?.total_text_length || 0
})

// 处理开始模拟按钮点击 — 先显示 pre-flight 估算
const handleStartSimulation = () => {
  if (preflightAgents.value > 0) {
    preflightVisible.value = true
  } else {
    // fallback: 没有 agent 数量时跳过 modal
    proceedStartSimulation()
  }
}

const proceedStartSimulation = () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.progress_detail) {
        ...
        // outputdetailedprogresslog (avoid duplicate) 
        ...
        // outputmessagelog (avoid duplicate) 
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // outputconfiggeneratephaselog (avoid duplicate) 
...
const loadPreparedData = async () => {
  ...
  // getconfig (usereal-timeinterface) 
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // confignot yetgenerate, beginpoll
...
/* simulationround countconfigstyle */
